...
/* This is not currently used as of 8/7/14 */
...
/* storeUsername  and registerKey are used */
...
{
    return [AmazonKeyChainWrapper getValueFromKeyChain:kKeychainExpirationDateIdentifier];
}
...
{
//    OSStatus keychainError = SecItemDelete((CFDictionaryRef)[AmazonKeyChainWrapper createKeychainDictionaryForKey:kKeychainUidIdentifier]);
//    
...
//    
    ...
    /* Register key */
    OSStatus keychainError = SecItemDelete((CFDictionaryRef)[AmazonKeyChainWrapper createKeychainDictionaryForKey : kKeychainKeyIdentifier]);
    if(keychainError != errSecSuccess && keychainError != errSecItemNotFound)
    {
    ...
    if(keychainError != errSecSuccess && keychainError != errSecItemNotFound)
    {
        AMZLogDebug(@"Keychain Key: kKeychainUsernameIdentifier, Error Code: %d", (int)keychainError);
        return keychainError;
    }
