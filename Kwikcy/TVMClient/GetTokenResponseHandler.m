...
-(Response *)handleResponse:(int)responseCode body:(NSString *)responseBody
{
    if (responseCode == 200) {
        ...
        NSString *accessKey      = [JSONUtilities getJSONElement:json element:ACCESS_KEY];
        NSString *secretKey      = [JSONUtilities getJSONElement:json element:SECRET_KEY];
        NSString *securityToken  = [JSONUtilities getJSONElement:json element:SECURITY_TOKEN];
        NSString *expirationDate = [JSONUtilities getJSONElement:json element:EXPIRATION_DATE];
